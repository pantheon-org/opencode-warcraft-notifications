...
| [Deploy Documentation](#deploy-documentation-workflow)        | Push to main       | ~2-5 min | Build and deploy docs          |
...
graph TB
    ...
    subgraph "Documentation Phase"
        AL[Push to main - docs/**] --> AM[Deploy Documentation]
        AM --> AN[Build with Astro]
        AN --> AO[Deploy to docs branch]
        AO --> AP[GitHub Pages serves site]
    end

    subgraph "Monitoring"
        AQ[Weekly Schedule] --> AR[Repo Config Check]
        AR --> AS{Config Valid?}
        AS -->|No| AT[Create Issue]
        AS -->|Yes| AU[Report Success]
    end
    ...
    style AM fill:#00bcd4
    style AR fill:#607d8b
```
