...
if (!GITHUB_TOKEN || !GITHUB_REPOSITORY) {
  console.error('❌ Missing required environment variables: GITHUB_TOKEN, GITHUB_REPOSITORY');
  process.exit(1);
}

async function main() {
  console.log('🤖 Auto-merge bot starting...');
  ...
  try {
    ...
    console.error('❌ Failed to read event payload');
    process.exit(1);
  }
  ...
  if (!prNumber) {
    console.log('ℹ️ No PR found for this event - exiting');
    return;
  }
  ...
  try {
    ...
    console.error(`❌ Failed to get PR details for #${prNumber}`);
    process.exit(1);
  }
  ...
  console.log('✅ This is a version sync PR - checking if ready to merge');

  // Check if PR is mergeable
  if (prData.mergeable !== 'MERGEABLE') {
    console.log(`⚠️ PR is not mergeable (status: ${prData.mergeable}) - skipping`);
    return;
  }
...
- ✅ Auto-merge enabled with squash and merge
